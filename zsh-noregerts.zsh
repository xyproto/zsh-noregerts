...
command_not_found() {
    ...
    # Get a list of packages that contain the command name anywhere in the file path
    packages=$(LC_ALL=C pacman -F $1 2>/dev/null | awk '{print $1}')
    if [ -z "$packages" ]; then
    ...
    for package in ${(f)packages}; do
        ...
        if pacman -F -l $package 2>/dev/null | grep -q "usr/bin/$1"; then
